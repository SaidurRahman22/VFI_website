...
## Backend integration (Phases 0–7 — LIVE)
...
**Partner identity + tenancy (Phase 6 — LIVE):** agency registration is a *reviewable application*, not an instant account. Partner auth pages (`vfi-partner-login.html` wizard+sign-in, `vfi-partner-forgot.html`, `vfi-partner-verify.html`, NEW `vfi-partner-reset.html`) post via `window.VFIApi`; state travels as an opaque `flow_id` (no `?email=`); demo disclaimers gone.
- **Tenant tables:** `partner_agencies` (THE tenant; staff-managed, AgencyStatus enum), `partner_agency_members` (seats — the one **agency-scoped** table: `agency_id` + `BelongsToAgency` + Postgres RLS), `partner_applications` (held for review). Tenant column is **`agency_id`** (matches the P1 trait).
- **TENANCY HARD GATE (proven live on Postgres):** the tenant id comes from the SESSION only (`EnsurePartner` sets `TenantContext` + `SET app.agency_id`), never a request param. Two nets: the Eloquent `BelongsToAgency` global scope (fail-closed `1=0` with no tenant) AND Postgres **RLS FORCE**. Verified on the VPS: with the app scope stripped, a raw query sees only its own tenant, and with no tenant set returns **0 rows**. Cross-tenant read is impossible even with a forged `?agency_id=`. Staff cross-tenant reads must `withoutGlobalScope(BelongsToAgencyScope)`.
- **Endpoints** (`routes/web.php`, web group): `POST /api/partner/register` (3-step wizard → pending user + application + OTP; enumeration-safe; duplicate-agency = soft staff signal), `/email/verify|/code|/change` (OTP reuse P4; **email-change is bound to the server `flow_id`, not `?email=`** — restarts flow, rate-limited 2/registration, anti-hijack), `/signin` (resolves an ACTIVE seat of an APPROVED agency → binds session; pending/suspended/rejected/disabled → 403 review-gate; wrong pw → generic 401), `/logout`, `/password/forgot|/reset/submit` (reset revokes ALL sessions across agencies), `GET /api/partner/me` (greeting per authenticated agency, not a global string) + `/members` (tenant-scoped).
- **Staff review = Filament `PartnerApplicationResource`** (super-admin + staff_partner_ops): Approve/Reject/Request-more-info → `PartnerReview` service. **Approve** mints, in one transaction, exactly one agency + one owner seat + the `partner_owner` role, activates the user, audits (`agency_approved`), emails. Suspend/close flips status (sign-in gate) + revokes member sessions. `PartnerAgencyMember` uses `SeatRole::role()` → PartnerOwner/PartnerCounsellor.
**Partner console core (Phase 7 — LIVE):** the 11 `partner-*.html` pages are guarded (`portal.js` calls `/api/partner/me` on load; 401 → `vfi-partner-login.html`), all carry `noindex`, and their data comes from tenant-scoped endpoints via `js/portal-data.js`.
- **ATTRIBUTION RULE (settled by the client, enforced in code):** ① cross-agency collision → **first agency wins**, a second agency's attempt is refused 409 (never silently re-parented); ② the manual `+ New Student` modal refuses **any** existing email — owned *or* self-signup ("keep separate"); ③ **only a QR self-registration** can claim an **unowned self-signup**, and only **after email verification** (anti-commission-farming). `students.agency_id` null = unowned; `Student::resolveFor` ADOPTS an agency-lead row so there's one row per person.
- **Endpoints** (all under `EnsurePartner`): `students` (create + paged/filtered/archived list), `applications` (create + list) + `dashboard/kpis` (GROUP BY status, server-side — never client-filtered) + `dashboard/deadlines` (today/tomorrow/7d/14d), `enquiries` (multipart, reusing the **Phase 5 scan-gate byte-for-byte** + single-use signed doc download), `referral-link` (+regenerate), `notifications` (+read), `resources` (a REAL server query — the old client filter dumped every row). PUBLIC: `GET /api/referral/{slug}` + `qr-register.html`.
- **`applications` pipeline:** `PipelineService::create/transition` is the single mutation path; every status move writes **exactly one append-only** `application_status_events` row (staff transitions land in P9). Fee debit is a Pennant-flagged no-op here.
- `js/portal.js`'s **shared modal handler was SPLIT first** (non-negotiable): `#ppRegForm` → students, `#ppProgForm` → enquiries; one failing can't break the other. Dashboard QR is now a **real scannable server-generated SVG** (`endroid/qr-code`) pointing at `qr-register.html?ref=<opaque slug>` (was a decorative SVG + a hardcoded `?ref=hakim` to the partner login).
- ⚠️ **Postgres-only landmine (fixed, remember this):** three flows must read a tenant-keyed row **before** any tenant is bound — partner sign-in's seat lookup, and the public referral resolve/capture/convert. `withoutGlobalScope()` only drops the *Eloquent* net; **RLS FORCE still hid every row**, silently refusing every partner sign-in and 404ing every QR. Fixed via `App\Support\RlsBypass` (closure-scoped `app.rls_bypass='on'`, reset in `finally`) + policy `USING (... OR rls_bypass)`. **`WITH CHECK` deliberately has NO bypass — writes still require a real tenant.** SQLite has no RLS, so the suite can't catch this class: **verify tenancy changes on the live Postgres.**

**Security (mandatory, enforced at the model layer so any write path is covered):** blog body = plain text (`strip_tags`); `ppQuicklinks`/`ppDocs` URLs scheme-allow-listed (`App\Support\UrlGuard`); tenancy scope + RLS (Postgres); argon2id; append-only audit/auth logs; uploads validated by magic-bytes not extension; backup restore is snapshotted + owner-gated; OTP/reset tokens hashed at rest + single-use; enumeration-safe register/login/forgot; server-side rate limits; student endpoints implicit-self (no IDOR); documents scan-gated on a private disk with single-use signed downloads + append-only access log; partner tenancy = session-only tenant id + Eloquent scope + Postgres RLS FORCE (proven fail-closed on live PG); partner email-change bound to server flow_id; console data + KPIs tenant-scoped and server-computed; attribution counts only after email verification. 211 backend tests passing.
